...
def print_normalized_importance(model, X_train):
    ...
    df_imp = pd.DataFrame(
        dict(zip(feat_names, importance)).items(), columns=["feature", "importance"]
    )
...
def prepare_training_data(
    df: pd.DataFrame, target_col: str, time_col: str, test_ratio: float = 0.2
):
    df = df.copy()
    try:
...
def train_and_evaluate(
    name, X_train, X_test, y_train, y_test, plot=True, save_dir="plots"
):
    ...
    mape = (
        np.mean(
            np.abs((y_test[nonzero_true] - preds[nonzero_true]) / y_test[nonzero_true])
        )
        * 100
    )
...
def evaluate_processing(data_path, processed_file, target_col, time_col):
    ...
    train_and_evaluate(
        "Preprocessed Data", X_proc_train, X_proc_test, y_proc_train, y_proc_test
    )


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Evaluate forecasting models on raw and processed data."
    )
